...
* **Left Sidebar Rail (Right-Aligned in RTL mode):** Fixed Width `72px`. Houses role-based navigation destinations resolved from `roleNavMap` (`default_bindings.dart`): `admin` → Sales, Inventory, Settings; `cashier` → Checkout, Sales. There is no Ingestion or Logs destination. The rail order defines the positional F-key mapping (rail[0]→F1, rail[1]→F2, rail[2]→F3).
...
* **Empty state:** `PhosphorIcons.package` Duotone (48px, grey.shade400) → headline → body copy → optional localized action. Inventory provides its add-product action; checkout provides an Inventory action when wired by the shell.
...
* **Row Selection:** A local `ValueNotifier<int> _selectedIndex` tracks which row is focused for keyboard navigation. The selected row renders with a distinct highlight state (accented background or border, matching the primary color `#007ACC`). `_editingIndex` tracks which row is in edit mode (-1 = none). `arrowDown`/`arrowUp`/`delete`/`enter` are handled by a global `HardwareKeyboard` handler registered by the widget — there is no cart `Focus` node, so the cart never steals focus from the scanner. `SelectNextCartItemIntent` increments `_selectedIndex`; `SelectPrevCartItemIntent` decrements it. Both clamp to valid range and wrap.
...
* **Discount field:** A `TextField` with `FilteringTextInputFormatter.digitsOnly`, hint `"0%"`. On every keystroke, `onChanged` parses the percent, clamps to 0-100, and dispatches `SetDiscount(percent.clamp(0, 100))` to `CheckoutBloc` in real-time. On `onSubmitted`, the field unfocuses and `FocusController.returnToScanner()` hands focus back to the scanner node. A warning icon (`Icons.warning_amber_rounded` — Material, no Phosphor equivalent) appears if the entered value exceeds 100, though the clamped value is still dispatched.
* **Discount focus loan:** Accepts `ValueNotifier<int>? discountFocusTrigger` plus `FocusController`. When incremented, `FocusController.requestFocusLoan(FocusZone.discount, _discountFocusNode)` requests focus on the discount `TextField` and all existing text is selected.
...
* **Layout Blocks:** Sectioned card layout using `Card` widgets with `_SettingsSection` wrapper, wrapped in a `SectionCard` notch title container (replacing previously used `AppBar`). Nine distinct sections stacked vertically in a `SingleChildScrollView` (`settings_workspace.dart:16-23`):
  ...
  9. **Reset All Data Section:** Subtitle text + red `ElevatedButton` triggering confirmation dialog, then clearing all relevant Hive boxes.
* **Save Mechanism:** Per-tab auto-save — each user interaction immediately fires a `SettingsBloc` event. No explicit "Apply Changes" button. Changes persist through the explicit Hive-backed repository.
...
**TableWorkspace (`TableWorkspace`):** `Column` with `ListView` of zone sections. Each zone = `SectionCard` with notch title (zone name) + `GridView` of `TableCard` widgets (max cross-axis extent 220px, 16px spacing). Zone order: dine-in zones first (Main Dining, Terrace, VIP), Takeaway Queue last. Zone header uses `TextStyles.heading3` with zone icon.
