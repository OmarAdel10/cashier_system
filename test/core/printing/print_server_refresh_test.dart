...
void main() {
  group('decidePublish', () {
    ...
    test('csproj present + first existing candidate newer than sources → none', () {
      final action = decidePublish(
        csprojExists: true,
        newestSourceModified: newestSource,
        candidateExes: [
          (path: r'C:\app\PrintServer.exe', modified: newestSource.add(const Duration(hours: 1))),
        ],
      );
      expect(action, PrintServerBuildAction.none);
    });

    test('csproj present + first candidate dated exactly equal to newest source → none', () {
      final action = decidePublish(
        csprojExists: true,
        newestSourceModified: newestSource,
        candidateExes: [
    ...
    test('csproj present + first candidate older than sources → publish', () {
      final action = decidePublish(
        ...
        newestSourceModified: newestSource,
        candidateExes: [
          (path: r'C:\app\PrintServer.exe', modified: newestSource.subtract(const Duration(days: 1))),
        ],
      );
    ...
    test('csproj present + no source mtimes recorded → publish (cannot verify freshness)', () {
      final action = decidePublish(
        csprojExists: true,
    ...
    test('csproj present + first candidate stale despite newer lower-priority exe → publish', () {
      final action = decidePublish(
        csprojExists: true,
        newestSourceModified: newestSource,
        candidateExes: [
    ...
    test('csproj present + first candidate fresh → none even with stale lower-priority exe', () {
      final action = decidePublish(
        csprojExists: true,
        newestSourceModified: newestSource,
        candidateExes: [
